...
const CHAPTER_APPLY_URL = "https://docs.google.com/forms/d/e/1FAIpQLSfrnxVoEypNkuKiM3rYdgzBWXqVX6aFy-w6gDyZF7GVJcaubA/viewform?usp=header";
...
function PhotoSlideshow({ photos, chapterName }) {
  ...
  return (
    <div style={{ position: "relative", borderRadius: "8px 8px 0 0", overflow: "hidden", background: "#D8E3DD" }}>
      <div style={{ width: "100%", paddingTop: "62%", position: "relative", overflow: "hidden" }}>
        <img
          src={photo.src}
        ...
        {photo.caption && (
          <div style={{
            ...
            <p style={{
              fontFamily: SANS, fontSize: 12, color: "#fff", margin: 0, lineHeight: 1.4,
              opacity: fade ? 1 : 0, transition: "opacity 0.18s ease",
            }}>
              {photo.caption}
            </p>
      ...
      {photos.length > 1 && (
        <>
          <button onClick={e => { e.stopPropagation(); go(-1); }} style={{ position: "absolute", left: 8, top: "50%", transform: "translateY(-50%)", background: "rgba(255,255,255,0.85)", border: "none", borderRadius: "50%", width: 28, height: 28, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, color: INK }}>‹</button>
          <button onClick={e => { e.stopPropagation(); go(1); }}  style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", background: "rgba(255,255,255,0.85)", border: "none", borderRadius: "50%", width: 28, height: 28, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, color: INK }}>›</button>
          <div style={{ position: "absolute", bottom: photo.caption ? 30 : 8, left: 0, right: 0, display: "flex", justifyContent: "center", gap: 5 }}>
            {photos.map((_, i) => (
              <button key={i} onClick={e => { e.stopPropagation(); setFade(false); setTimeout(() => { setIdx(i); setFade(true); }, 160); }} style={{ width: i === idx ? 16 : 6, height: 6, borderRadius: 3, background: i === idx ? "#fff" : "rgba(255,255,255,0.5)", border: "none", padding: 0, cursor: "pointer", transition: "width 0.2s, background 0.2s" }} />
            ))}
          </div>
          <div style={{ position: "absolute", top: 8, right: 8, background: "rgba(22,34,29,0.55)", borderRadius: 100, padding: "3px 9px", fontFamily: SANS, fontSize: 11, color: "#fff" }}>
            {idx + 1} / {photos.length}
          </div>
        </>
      )}
...
function ChapterCard({ c }) {
  const isInternational = c.region === "International";
  ...
  return (
    <div style={{
      ...
      {hasPhotos && <PhotoSlideshow photos={c.photos} chapterName={c.name} />}

      <div style={{ padding: 20 }}>
        {isInternational && (
          <div style={{
            display: "inline-flex", alignItems: "center", gap: 5,
            background: ACCENT_SOFT, borderRadius: 100,
    ...
    <div style={{ paddingTop: 64 }}>
      <Container style={{ padding: "72px 32px 16px" }}>
        ...
        <p style={{ fontFamily: SANS, fontSize: 16, color: INK_SOFT, maxWidth: 580, lineHeight: 1.7, marginBottom: 32 }}>
          ...
          communities across the USA and around the world. Find a chapter near you — or start your own.
        </p>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 40 }}>
          <a
            ...
            style={{
              ...
              padding: "12px 24px", borderRadius: 4, display: "inline-block",
            }}
          >
            Start a chapter &rarr;
          </a>
          <a
            href={CHAPTER_APPLY_URL}
            target="_blank"
            rel="noreferrer"
            style={{
              fontFamily: SANS, fontSize: 14, fontWeight: 500,
          ...
          >
            Interested in becoming a chapter? Apply here →
          </a>
        </div>
